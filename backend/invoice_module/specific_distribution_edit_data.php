...
if (intval($quantity) > intval($quantityToCut)) {
    ...
    for ($i = 0; $i < count($data); $i++) {
        ...
        $pack_quantity = intval($quantity) - intval($quantityToCut);
        ...
        if ($pack_quantity <= $resultQuantity && ($resultQuantity - $resultSoldQuantity) != 0) {
            $newAmount = $resultSoldQuantity + $pack_quantity;
            ...
            if (($resultQuantity - $resultSoldQuantity) != 0) {
                $con->query($query);
            }
    ...
    $response = array("status" => FALSE, "modal" => FALSE, "error" => "Can not decrease sold products quantity!");
    echo json_encode($response);
}
// else {
//     $query = "";
