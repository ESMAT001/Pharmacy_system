                        ...
                        }
                        {
                            filteredCustomer.length >0 && <thead>
                                <tr>
                                    ...
                                    <th className="border border-black">Total Paid</th>
                                    <th className="border border-black">Remain</th>
                                </tr>
                            </thead>
                        }
                        {
                            filteredCustomer.length >0 && filteredCustomer.map((list,id)=>(
                                <tbody key={id}>
                                    <tr className="hover:bg-blue-300 cursor-pointer">
                                        ...
                                        <th className="border border-black">{list.total_of_all}</th>
                                        <th className="border border-black">{list.entry_date} --- {DateChangerFromGerordgiaToSolarDate(list.entry_date)}</th>

                                        <th className="border border-black">{list.total_paid_amount_from_previous}</th>
                                        <th className="border border-black">{list.total_remain_after_calculate}</th>
                                    </tr>
                                </tbody>
                            ))
                        }
