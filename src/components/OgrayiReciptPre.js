...
const [SearchForCustomerInput, setSearchForCustomerInput] = useState("")

const filteredCustomer=CustomerListForSelect.filter(list=>list.customer_name.toLowerCase().includes(SearchForCustomerInput.toLowerCase()))

const CustomerModalDataClear=()=>{
    setCustomerReciptListModalVisibility(false)
    setTotalAmount(0)
    setTotalPaid(0)
    setleftAmount(0)
    setSearchForCustomerInput("")
...
<div >
<div className="inline float-right font-bold cursor-pointer " onClick={CustomerModalDataClear}>X</div>

<div className="text-center" className="inline pb-3">
<input type="text" value={SearchForCustomerInput} onChange={(e)=>setSearchForCustomerInput(e.target.value)} placeholder="Search Customer" className="border border-blue-400 px-4 py-2 focus:outline-none rounded-md font-bold w-80 mb-3" />
<br />
