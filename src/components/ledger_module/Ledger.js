...
const selectHandler=(e)=>{
    ...
    setcustomerListAfterSelectDay([])
    setDayOfTheWeek(0)

    
}
const [VisitorNameForTitle, setVisitorNameForTitle] = useState("")
const [VisitorLastnameForTitle, setVisitorLastnameForTitle] = useState("")
const setVisitorDetails=(visitorDetails)=>{
    visitorDetails.map(list=>{
        setVisitorNameForTitle(list.name)
        setVisitorLastnameForTitle(list.last_name)
    })
}

useEffect(() => {
   const filteredList=visitorList.filter(list=>list.visitor_id.includes(visitorId))

    setVisitorDetails(filteredList)
...
return (
        ...
        <div>
            ...
            <div className="text-center">
                ...
                <select value={DayOfTheWeek}  className="border-2 font-bold focus:outline-none border-blue-400 rounded-md px-3 py-1 mx-2 " onChange={DayHandler} >
              ...
              <p className="font-semibold  pb-4">(ثبت و راجستر شده وزرات صحت عامه)</p>
...

                    {
                        ...
                        <table className="w-full  mt-4  border-black ">
                            <thead>
                                <th className="border border-black" colspan ="4" >{VisitorNameForTitle} {VisitorLastnameForTitle} Date: {formatDate()} --- {DateChangerFromGerordgiaToSolarDate(formatDate())}</th>
                            </thead>
                        <thead>
                            ...
                            <th className="border border-black no-print">Action</th>
                                    ...
                                    {
                                        customerListAfterSelectDay.length>0 && customerListAfterSelectDay.map((list,id)=>(
                                           <tbody>
                                           <tr>
